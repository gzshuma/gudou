					...
					<div class="episodes-reset" :class="episodesData.length>1 ? '':'dis-hide'">
						<el-tabs class="juji-box">
							<el-tab-pane label="1-24">
								<span class="juji-list" v-for="(v,index) in episodesData" v-if="index<24" :key="v.programID" @click="getUrl(v,index,$event)">
									<el-tooltip class="item" effect="dark" :content="v.titleName" placement="top">
										<el-button :data-url="v.movieUrl[0]['4']">{{index+1}}</el-button>
									</el-tooltip>
								</span>
							</el-tab-pane>
							<el-tab-pane label="25-48" v-if="episodesData.length>=24&&episodesData.length<48">
								<span class="juji-list" v-for="(v,index) in episodesData" v-if="index>=20&&index<40" :key="v.programID" @click="getNum($event,index)">
									<el-tooltip class="item" effect="dark" :content="v.titleName" placement="top">
							...
							<el-tab-pane label="49-72" v-if="episodesData.length>=48&&episodesData.length<72">
								<span class="juji-list" v-for="(v,index) in episodesData" v-if="index>=40&&index<60" :key="v.programID" @click="getNum($event,index)">
									<el-tooltip class="item" effect="dark" :content="v.titleName" placement="top">
										<el-button>{{index+1}}</el-button>
									</el-tooltip>
								</span>
...
export default {
	...
	methods: {
		...
		getUrl (val, index, el)  {
			// console.log(val.movieUrl[0]['4'])
			let a = val.movieUrl[0]['4'].split('8070')
...
.el-tabs__header { border-bottom-color: #363636; }
.episodes-top { position: relative; }
.episodes-top-ri { position: absolute; top: 0; right: 50px; font-size: 20px; color: #888; }
.episodes-top-ri i { display: block; font-size: 14px; margin-top: 5px; }
.el-tab-pane { width: 366px; }
.juji-list { float: left; width: 60px; height: 40px; border-radius: 5px; line-height: 48px; background: #f0f0f0; text-align: center; border-radius: 0; background: none; border: 1px solid #363636; border-width: 0 1px 1px 0; cursor: pointer; }
.dis-hide { display: none; }
</style>
<style>
.palyer-ri .el-tabs__item { color: #aaa; font-size: 16px; }
...
.juji-list.cur { background: #ff9c01; }
.juji-list.cur .el-button { color: #fff; }
