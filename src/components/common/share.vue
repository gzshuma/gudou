...
import { paramFunction, addAllPlayLiveUrl, delAllPlayLiveUrl, addAppointCollectionUrl, delleteAppointCollectionUrl, praiseUrl } from '@/axios/api'
import $ from 'jquery'
export default {
  props:{
    collectData:{
  ...
  methods: {
      ...
      delDian(){
        ...
        this.$http({
            method: 'post',
            url: delleteAppointCollectionUrl(),
            params: paramFunction(),
            //post用data
            data:{
              programID: self.collectData.id,
